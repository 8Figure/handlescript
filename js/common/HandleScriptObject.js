function HandleScriptObject(templateName, divID, jsonUrl, jsFile, css, delayLoad)  
{
	...
	this.getTemplate = function()
	{
		...
		var templateURL = window.location.protocol + "//" + window.location.host + "/mustache_templates/controls/" + _templateName;
...
function HandleScriptContent(id) {
	var _id = id;
	...
	this.load = function()
	{
		...
		success: function(data)
		{
			...
			var name = pathParts[pathParts.length - 1].replace(".php", "").toLowerCase();
			...
			if (_children.hasOwnProperty(key))
			{
				if(_children[key].getDelayLoad() == "false")
				{
					...
					if (_id != null && _id != "")
					{
						...
						//the html page and instead use the children to render everything
						
						$(_id).append($('<div/>').attr('id', HandleScriptObject.generateID()).html(_children[key].getHtml()));
					}
					else
					{
